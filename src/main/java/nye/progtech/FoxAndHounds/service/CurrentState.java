...
public class CurrentState {
    ...
    public CurrentState(int foxColumn) {
        this();
        fox = new Coordinates(0, foxColumn);
    }


    private CurrentState(CurrentState state) {
        if (state.fox != null) {
    ...
    public Vector<CurrentState> foxNeighbours(boolean includeNulls) {
        Vector<CurrentState> neighbours = new Vector<CurrentState>();
        if (fox == null) {
            for (int i = 0; i < 4; ++i) {
                neighbours.add(new CurrentState(i));
            }
            return neighbours;
        }
        Coordinates moved;
        ...
        CurrentState sameColumnState = (isHoundAt(moved) ? null : moveFox(moved));
        if (fox.getRow() % 2 == 0) {
    ...
    public Vector<CurrentState> houndsNeighbours(boolean includeNulls) {
        Vector<CurrentState> neighbours = new Vector<CurrentState>();
        for (int i = 0; i < 4; ++i) {
            Coordinates moved;
    ...
    public CurrentState moveFox(Coordinates coordinates) {
        CurrentState state = new CurrentState(this);
        state.fox = coordinates;
        return state;
    }


    private CurrentState moveHound(int houndIndex, Coordinates coordinates) {
        CurrentState state = new CurrentState(this);
    ...
    public boolean equals(CurrentState state) {
        for (int i = 0; i < 4; ++i) {
